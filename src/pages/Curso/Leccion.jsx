...
export default function Leccion() {
  ...
  const guardarProgreso = async () => {
    if (progresoCurso.includes(leccionId)) return true;
    ...
    try {
      ...
      setGuardando(false);
      await recargarProgreso(); // 🔄 SIEMPRE
    }
  };
  ...
  const progresoCursoPct = Math.round(
    (progresoCurso.length / totalLeccionesCurso) * 100
  );

  const leccionCompletada = progresoCurso.includes(leccionId);
